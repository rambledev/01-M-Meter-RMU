...
### 2026-09-14 — local dev (browser/OS ไม่ระบุ), มิเตอร์กลไกแบบล้อหมุน (odometer wheel)

**สภาพแวดล้อม**: ทดสอบผ่าน Upload Image Mode ด้วยภาพถ่ายจริงของมิเตอร์ PG KILOWATT-HOUR METER รุ่น IPG (ฟอนต์ตัวเลขแบบล้อกลไก ไม่ใช่ seven-segment/LCD) ค่าที่ถูกต้องคือ 23188 (5 หลัก) — หลักสุดท้าย (หลักหน่วย) เป็นล้อที่หมุนตลอดเวลา มักเบลอในภาพนิ่ง จึงทดสอบเฉพาะ 4 หลักแรก "2318" ที่อ่านนิ่งได้
**Config ที่ใช้ทดสอบ**: เปลี่ยนระหว่างทาง 3 รอบ (ดูรายละเอียดด้านล่าง)
**Calibration Checklist**: ไม่ได้กรอกตารางหัวข้อ 5 แยก — สรุปรวมไว้ในนี้แทนเพราะเป็น session แรกที่ใช้หา root cause ไม่ใช่แค่ tune ค่า

**ลำดับปัญหาที่พบและแก้**:

| รอบ | อาการ | ROI ถูกต้องไหม | สาเหตุที่พบ | การแก้ |
|---|---|---|---|---|
| 1 | `value` มีตัวอักษร/สัญลักษณ์ปน เช่น `12318 0` | ยังไม่ได้ปรับ (กรอบเดิมรวมไอคอน/สกรูข้างเคียง) | ROI กว้างเกิน รวมไอคอนหมุน/สกรูที่ไม่ใช่ตัวเลข | ผู้ใช้ปรับกรอบ ROI ให้แคบลงเฉพาะ "2318" |
| 2 | `value = "EERE"` (ตัวอักษรล้วน ไม่ใช่ตัวเลขเลย) confidence 56% | ถูกต้องแล้ว (crop สะอาด) | Tesseract (LSTM engine) อ่านฟอนต์ล้อกลไกผิดเป็นตัวอักษร — สงสัยว่า preprocessing เดิม (Otsu threshold บังคับขาวดำล้วน) ทำลายรายละเอียดที่ LSTM ต้องใช้ | ลบขั้นตอน threshold ออกจาก `meterPreprocess.ts` เหลือ grayscale+contrast+denoise (ไม่บังคับขาวดำ) |
| 3 | `value = "2318"` (**ถูกต้อง**) confidence 50% — ยังต่ำกว่า `previewConfidence` เดิม (60%) เลยไม่ยอมรับค่า | ถูกต้อง | ตัวเลขอ่านถูกแล้วจริง แค่ threshold confidence ตั้งไว้สูงเกินสำหรับฟอนต์นี้ | ลด `previewConfidence` จาก 0.6 → 0.45 ใน `lib/calibration/config.ts` |

**สรุป/ข้อเสนอแก้ config**: `previewConfidence` 0.6 เดิมสูงเกินไปสำหรับฟอนต์มิเตอร์แบบล้อกลไก (odometer wheel) — ค่าที่อ่านถูกจริงได้แค่ ~50% — ปรับเป็น 0.45 แล้ว (มี margin เหลือ ~5% จากค่าที่วัดได้จริงครั้งนี้) `stableConfidence` (0.85, ใช้เฉพาะ live-preview auto-fill ไม่ใช่ปุ่ม "อ่านตัวเลข" ที่ใช้ทดสอบรอบนี้) **ยังไม่ได้ปรับ/ทดสอบ** — ยังไม่รู้ว่า live mode จะขึ้น stable ✓ ได้จริงไหมกับฟอนต์นี้ ต้องทดสอบเพิ่ม
**Action**: แก้ `config.ts` แล้ว (`previewConfidence: 0.45`) + แก้ `meterPreprocess.ts` (ตัด threshold step) — **ยังไม่ commit/push** ผู้ใช้ขอทดสอบให้มั่นใจก่อน

**ยังไม่ได้ทดสอบ/ข้อจำกัดที่รู้อยู่**:
- หลักที่ 5 (หลักหน่วย, ล้อหมุน) — เบลอโดยธรรมชาติของมิเตอร์แบบนี้ ไม่คาดว่า OCR จะอ่านได้แน่นอน ต้องพิมพ์เองเสมอสำหรับหลักนี้
- Live real-time preview (Camera Mode/ตอนขยับ ROI ใน Upload editor) ยังไม่ได้ยืนยันว่าจะขึ้นสถานะ "stable ✓" ได้จริง (ต้องผ่าน `stableConfidence` 0.85 × 3 ครั้งติด ซึ่งสูงกว่า confidence ที่วัดได้จริงมาก) — ทดสอบรอบนี้ผ่านแค่ปุ่ม "อ่านตัวเลข" (manual, เช็ค `previewConfidence` อย่างเดียว)
- ยังไม่ได้ลองมิเตอร์รุ่น/ยี่ห้ออื่น — ค่า 0.45 นี้ calibrate จากมิเตอร์ตัวเดียว ฟอนต์เดียว อาจต้องปรับอีกเมื่อเจอมิเตอร์แบบอื่น
